...
# 读取 Excel 文件
df = pd.read_excel(r'C:\Users\Zhen\Downloads\专用设备清单.xlsx')
...
for index, row in df.iterrows():
    ...
    device_name = str(row['名称']) if pd.notna(row['名称']) else None
    device_code = str(row['设备编号（*）']) if pd.notna(row['设备编号（*）']) else None
    ...
    image_url = str(row['图片（一项设备类名添加一份图片）']) if pd.notna(row['图片（一项设备类名添加一份图片）']) else None
    
    # 插入数据
    sql = """
    INSERT INTO device (
        seq_no, device_code, device_name, sn_code, brand, model_spec,
